...
upload.start = {
	...
	local: function(files) {
		...
		const process = function(files, file) {
			...
			const finish = function() {
				...
				if (album.getID()===false) ProrigoGallery.goto('0')
			...
			xhr.onload = function() {
				...
				if (xhr.status===200 && isNumber(data)) {
					...
					if (data.substr(0, 6)==='Error:') {
						...
						// Throw error
						if (error===true) ProrigoGallery.error('Upload failed. Server returned an error!', xhr, data)
						...
						// Throw error
						if (error===true) ProrigoGallery.error('Upload failed. Server returned a warning!', xhr, data)
						...
						// Throw error
						if (error===true) ProrigoGallery.error('Upload failed. Server returned an unkown error!', xhr, data)
		...
		window.onbeforeunload = function() { return 'ProrigoGallery is currently uploading!' }
	...
	url: function(url = '') {
		...
		basicModal.show({
			body: ProrigoGallery.html`<p>Please enter the direct link to a photo to import it: <input class='text' name='link' type='text' placeholder='http://' value='$${ url }'></p>`,
			buttons: {
				action: {
					title: 'Import',
					fn: action
	...
	server: function() {
			...
			upload.show('Importing from server', files, function() {
				...
				api.post('Import::server', params, function(data) {
					...
					if (data==='Notice: Import only contained albums!') {
						...
						if (visible.albums()) ProrigoGallery.load()
						else                  ProrigoGallery.goto()
						...
						// Log error
						ProrigoGallery.error('Could not start import because the folder was empty!', params, data)
		...
		basicModal.show({
			body: ProrigoGallery.html`<p>This action will import all photos, folders and sub-folders which are located in the following directory. The <b>original files will be deleted</b> after the import when possible. <input class='text' name='path' type='text' maxlength='100' placeholder='Absolute path to directory' value='$${ ProrigoGallery.location }uploads/import/'></p>`,
			buttons: {
				action: {
					title: 'Import',
					fn: action
