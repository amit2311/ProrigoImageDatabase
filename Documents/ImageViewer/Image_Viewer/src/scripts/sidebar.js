...
sidebar.toggle = function() {
	...
	if (visible.sidebar() || visible.sidebarbutton()) {
		...
		ProrigoGallery.content.toggleClass('content--sidebar')
...
sidebar.changeAttr = function(attr, value = '-', dangerouslySetInnerHTML = false) {
	...
	// Escape value
	if (dangerouslySetInnerHTML===false) value = ProrigoGallery.escapeHTML(value)
...
sidebar.createStructure.photo = function(data) {
	...
	if (ProrigoGallery.publicMode===false) editable = true
...
sidebar.render = function(structure) {
	...
	let renderDefault = function(section) {
		...
		section.rows.forEach(function(row) {
			...
			value = ProrigoGallery.html`<span class='attr_$${ row.title.toLowerCase() }'>$${ value }</span>`

			// Add edit-icon to the value when editable
			if (row.editable===true) value += ' ' + build.editIcon('edit_' + row.title.toLowerCase())

			_html += ProrigoGallery.html`
			         <tr>
			             <td>$${ row.title }</td>
			             <td>${ value }</td>
			         </tr>
