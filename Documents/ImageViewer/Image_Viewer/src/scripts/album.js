...
album.load = function(albumID, refresh = false) {
	...
	password.get(albumID, function() {
		...
		if (refresh===false) ProrigoGallery.animate('.content', 'contentZoomOut')
		...
		api.post('Album::get', params, function(data) {
			...
			// Skip delay when opening a blank ProrigoGallery
			...
			setTimeout(() => {
				...
				if (refresh===false) {
					ProrigoGallery.animate(ProrigoGallery.content, 'contentZoomIn')
	...
	const action = function(data) {
		...
		api.post('Album::add', params, function(data) {
			...
			if (data!==false && isNumber(data)) {
				albums.refresh()
				ProrigoGallery.goto(data)
			} else {
				ProrigoGallery.error(null, params, data)
			}
...
album.delete = function(albumIDs) {
	...
	if (albumIDs.toString()==='0') {
		...
		msg = ProrigoGallery.html`<p>Are you sure you want to delete the album '$${ albumTitle }' and all of the photos it contains? This action can't be undone!</p>`
		...
		msg = ProrigoGallery.html`<p>Are you sure you want to delete all $${ albumIDs.length } selected albums and all of the photos they contain? This action can't be undone!</p>`
...
album.setTitle = function(albumIDs) {
	...
	if (albumIDs.length===1) msg = ProrigoGallery.html`<p>Enter a new title for this album: ${ input }</p>`
	else                     msg = ProrigoGallery.html`<p>Enter a title for all $${ albumIDs.length } selected albums: ${ input }</p>`
...
album.setDescription = function(albumID) {
	...
	basicModal.show({
		body: ProrigoGallery.html`<p>Please enter a description for this album: <input class='text' name='description' type='text' maxlength='800' placeholder='Description' value='$${ oldDescription }'></p>`,
		buttons: {
			action: {
		...
		let msg = `
		          ...
		          <form>
		              ...
		              <div class='choice'>
		                  ...
		                  <p>Visitors of your ProrigoGallery can download this album.</p>
		              </div>
		              <div class='choice'>
		                  <label>
...
album.merge = function(albumIDs) {
	...
	if (albumIDs.length===2) {
		...
		msg = ProrigoGallery.html`<p>Are you sure you want to merge the album '$${ sTitle }' into the album '$${ title }'?</p>`

	} else {

		msg = ProrigoGallery.html`<p>Are you sure you want to merge all selected albums into the album '$${ title }'?</p>`
