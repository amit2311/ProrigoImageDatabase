...
photo.previous = function(animate) {
	...
	if (photo.getID()!==false &&
		...
		setTimeout(() => {
			...
			ProrigoGallery.goto(album.getID() + '/' + album.json.content[photo.getID()].previousPhoto)
		}, delay)
...
photo.next = function(animate) {
	...
	if (photo.getID()!==false &&
		...
		setTimeout(() => {
			if (photo.getID()===false) return false
			ProrigoGallery.goto(album.getID() + '/' + album.json.content[photo.getID()].nextPhoto)
		}, delay)
...
photo.duplicate = function(photoIDs) {
	...
	api.post('Photo::duplicate', params, function(data) {
		...
		if (data!==true) ProrigoGallery.error(null, params, data)
...
photo.delete = function(photoIDs) {
	...
	action.fn = function() {
		...
		if (visible.photo() && nextPhoto!=null && nextPhoto!==photo.getID()) ProrigoGallery.goto(album.getID() + '/' + nextPhoto)
		else if (!visible.albums())                                          ProrigoGallery.goto(album.getID())
	...
	if (photoIDs.length===1) {
		...
		msg = ProrigoGallery.html`<p>Are you sure you want to delete the photo '$${ photoTitle }'? This action can't be undone!</p>`
		...
		msg = ProrigoGallery.html`<p>Are you sure you want to delete all $${ photoIDs.length } selected photo? This action can't be undone!</p>`
...
photo.setTitle = function(photoIDs) {
	...
	let input = ProrigoGallery.html`<input class='text' name='title' type='text' maxlength='50' placeholder='Title' value='$${ oldTitle }'>`

	if (photoIDs.length===1) msg = ProrigoGallery.html`<p>Enter a new title for this photo: ${ input }</p>`
	else                     msg = ProrigoGallery.html`<p>Enter a title for all $${ photoIDs.length } selected photos: ${ input }</p>`
...
photo.setPublic = function(photoID, e) {
	...
	if (photo.json.public==='2') {
		...
		const action = function() {
			...
			basicModal.close()
			ProrigoGallery.goto(photo.json.original_album)
...
photo.setDescription = function(photoID) {
	...
	basicModal.show({
		body: ProrigoGallery.html`<p>Enter a description for this photo: <input class='text' name='description' type='text' maxlength='800' placeholder='Description' value='$${ oldDescription }'></p>`,
		buttons: {
			action: {
...
photo.editTags = function(photoIDs) {
	...
	let input = ProrigoGallery.html`<input class='text' name='tags' type='text' maxlength='800' placeholder='Tags' value='$${ oldTags }'>`

	if (photoIDs.length===1) msg = ProrigoGallery.html`<p>Enter your tags for this photo. You can add multiple tags by separating them with a comma: ${ input }</p>`
	else                     msg = ProrigoGallery.html`<p>Enter your tags for all $${ photoIDs.length } selected photos. Existing tags will be overwritten. You can add multiple tags by separating them with a comma: ${ input }</p>`
...
photo.share = function(photoID, service) {
	...
	switch (service) {
		...
		case 'mail':
			...
			break
		case 'dropbox':
			ProrigoGallery.loadDropbox(function() {
...
photo.getArchive = function(photoID) {
	...
	if (ProrigoGallery.publicMode===true) link += `&password=${ encodeURIComponent(password.value) }`
