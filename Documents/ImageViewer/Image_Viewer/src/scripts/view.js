...
view.albums = {
	...
	title: function() {
		...
		ProrigoGallery.setTitle('Albums', false)
	...
	content: {
		...
		init: function() {
			...
			// Smart Albums
			if (ProrigoGallery.publicMode===false) {
			...
			if (albums.json.albums && albums.json.num!==0) {
				...
				// Add divider
				if (ProrigoGallery.publicMode===false) albumsData = build.divider('Albums') + albumsData

			}

			if (smartData==='' && albumsData==='') {
				ProrigoGallery.content.html('')
				$('body').append(build.no_content('eye'))
			} else {
				ProrigoGallery.content.html(smartData + albumsData)
			}
		...
		title: function(albumID) {
			...
			title = ProrigoGallery.escapeHTML(title)
		...
		delete: function(albumID) {
			...
			$('.album[data-id="' + albumID + '"]').css('opacity', 0).animate({
				...
				$(this).remove()
				if (albums.json.num<=0) ProrigoGallery.content.find('.divider:last-child').remove()
			})
...
view.album = {
	...
	title: function() {
		...
		if ((visible.album() || !album.json.init) && !visible.photo()) {
			...
			switch (album.getID()) {
				case 'f':
					ProrigoGallery.setTitle('Starred', false)
					break
				case 's':
					ProrigoGallery.setTitle('Public', false)
					break
				case 'r':
					ProrigoGallery.setTitle('Recent', false)
					break
				case '0':
					ProrigoGallery.setTitle('Unsorted', false)
					break
				default:
					if (album.json.init) sidebar.changeAttr('title', album.json.title)
					ProrigoGallery.setTitle(album.json.title, true)
					break
			}
	...
	content: {
		...
		init: function() {
			...
			// Add photos to view
			ProrigoGallery.content.html(photosData)
...
view.photo = {
	...
	show: function() {
		...
		// Change header
		ProrigoGallery.content.addClass('view')
		...
		ProrigoGallery.animate(ProrigoGallery.imageview, 'fadeIn')
	...
	hide: function() {
		...
		ProrigoGallery.content.removeClass('view')
		...
		// Hide Photo
		ProrigoGallery.animate(ProrigoGallery.imageview, 'fadeOut')
		setTimeout(() => {
			ProrigoGallery.imageview.hide()
			view.album.sidebar()
		}, 300)
	...
	title: function() {
		...
		ProrigoGallery.setTitle(photo.json.title, true)
	...
	photo: function() {
		...
		ProrigoGallery.imageview.html(build.imageview(photo.json, visible.header()))

		let $nextArrow     = ProrigoGallery.imageview.find('a#next')
		let $previousArrow = ProrigoGallery.imageview.find('a#previous')
		...
		if (hasNext===false || ProrigoGallery.viewMode===true) {
			...
			$nextArrow.css('background-image', ProrigoGallery.html`linear-gradient(to bottom, rgba(0, 0, 0, .4), rgba(0, 0, 0, .4)), url("$${ nextPhoto.thumbUrl }")`)

		}

		if (hasPrevious===false || ProrigoGallery.viewMode===true) {
			...
			$previousArrow.css('background-image', ProrigoGallery.html`linear-gradient(to bottom, rgba(0, 0, 0, .4), rgba(0, 0, 0, .4)), url("$${ previousPhoto.thumbUrl }")`)
