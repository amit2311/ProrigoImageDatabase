...
header.bind = function() {
	...
	// Event Name
	let eventName = ProrigoGallery.getEventName()
	...
	header.dom('#button_signin')      .on(eventName, ProrigoGallery.loginDialog)
	...
	header.dom('.header__hostedwith') .on(eventName, function() { window.open(ProrigoGallery.website) })
	...
	header.dom('#button_back_home')   .on(eventName, function() { ProrigoGallery.goto() })
	header.dom('#button_back')        .on(eventName, function() { ProrigoGallery.goto(album.getID()) })
...
header.show = function() {
	...
	ProrigoGallery.imageview.removeClass('full')
...
header.hide = function(e) {
	...
	if (visible.photo() && !visible.sidebar() && !visible.contextMenu() && basicModal.visible()===false) {
		...
		ProrigoGallery.imageview.addClass('full')
...
header.setTitle = function(title = 'Untitled') {
	...
	let html   = ProrigoGallery.html`$${ title }${ build.iconic('caret-bottom') }`
...
header.setMode = function(mode) {
	...
	if (mode==='albums' && ProrigoGallery.publicMode===true) mode = 'public'
	...
	switch (mode) {
		...
		case 'album':
			...
			if (ProrigoGallery.publicMode===true && album.json.downloadable==='0') $('#button_archive').hide()
...
header.setEditable = function(editable) {
	...
	if (ProrigoGallery.publicMode===true) editable = false
