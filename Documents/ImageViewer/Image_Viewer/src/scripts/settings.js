...
settings.createConfig = function() {
	...
	const action = function(data) {
		...
		if (dbName.length<1) dbName = 'ProrigoGallery'
	...
	let msg = `
	          ...
	          <p>
	              ProrigoGallery will create its own database. If required, you can enter the name of an existing database instead:
...
settings.setSorting = function() {
	...
	const action = function() {
		...
		api.post('Settings::setSorting', params, function(data) {
			...
			if (data===true) {
				ProrigoGallery.sortingAlbums = 'ORDER BY ' + sortingAlbums[0] + ' ' + sortingAlbums[1]
				ProrigoGallery.sortingPhotos = 'ORDER BY ' + sortingPhotos[0] + ' ' + sortingPhotos[1]
				ProrigoGallery.load()
			} else ProrigoGallery.error(null, params, data)
	...
	if (ProrigoGallery.sortingAlbums!=='') {

		sortingAlbums = ProrigoGallery.sortingAlbums.replace('ORDER BY ', '').split(' ')
	...
	if (ProrigoGallery.sortingPhotos!=='') {

		sortingPhotos = ProrigoGallery.sortingPhotos.replace('ORDER BY ', '').split(' ')
...
settings.setDropboxKey = function(callback) {
	...
	const action = function(data) {
		...
		api.post('Settings::setDropboxKey', { key }, function(data) {
			...
			if (data===true) {
				ProrigoGallery.dropboxKey = key
				if (callback) ProrigoGallery.loadDropbox(callback)
			} else ProrigoGallery.error(null, params, data)
	...
	let msg = ProrigoGallery.html`
	          <p>
	              ...
	              <input class='text' name='key' type='text' placeholder='Dropbox API Key' value='$${ ProrigoGallery.dropboxKey }'>
	          </p>
	          `
