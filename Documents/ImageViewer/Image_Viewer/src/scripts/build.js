...
build.iconic = function(icon, classes = '') {
	...
	html += ProrigoGallery.html`<svg class='iconic $${ classes }'><use xlink:href='#$${ icon }' /></svg>`
...
build.divider = function(title) {
	...
	html += ProrigoGallery.html`<div class='divider'><h1>$${ title }</h1></div>`
...
build.editIcon = function(id) {
	...
	html += ProrigoGallery.html`<div id='$${ id }' class='edit'>${ build.iconic('pencil') }</div>`
...
build.multiselect = function(top, left) {
	...
	return ProrigoGallery.html`<div id='multiselect' style='top: $${ top }px; left: $${ left }px;'></div>`
...
build.album = function(data) {
	...
	let { path: retinaThumbUrl, isPhoto } = ProrigoGallery.retinize(data.thumbs[0])

	html += ProrigoGallery.html`
...
build.photo = function(data) {
	...
	let { path: retinaThumbUrl } = ProrigoGallery.retinize(data.thumbUrl)
	...
	if (data.cameraDate==='1') html += ProrigoGallery.html`<a><span title='Camera Date'>${ build.iconic('camera-slr') }</span>$${ data.sysdate }</a>`
	else                       html += ProrigoGallery.html`<a>$${ data.sysdate }</a>`
...
build.imageview = function(data, visibleControls) {
	...
	if (hasMedium===false) {
		...
		html += ProrigoGallery.html`<img id='image' class='$${ visibleControls===true ? '' : 'full' }' src='$${ data.url }' draggable='false'>`

	} else {

		html += ProrigoGallery.html`<img id='image' class='$${ visibleControls===true ? '' : 'full' }' src='$${ data.url }' srcset='$${ data.medium } 1920w, $${ data.url } $${ data.width }w' draggable='false'>`
...
build.tags = function(tags) {
	...
	if (tags!=='') {
		...
		tags.forEach(function(tag, index, array) {
			html += ProrigoGallery.html`<a class='tag'>$${ tag }<span data-index='$${ index }'>${ build.iconic('x') }</span></a>`
		})
