...
contextMenu.settings = function(e) {
	...
	let items = [
		...
		{ },
		{ title: build.iconic('info') + 'About ProrigoGallery', fn: () => window.open(ProrigoGallery.website) },
		...
		{ },
		{ title: build.iconic('account-logout') + 'Sign Out', fn: ProrigoGallery.logout }
	]
	...
	api.post('Albums::get', {}, function(data) {
		...
		if (data.albums && data.num>1) {
			...
			$.each(data.albums, function() {
				...
				let html = ProrigoGallery.html`<img class='cover' width='16' height='16' src='$${ this.thumbs[0] }'><div class='title'>$${ this.title }</div>`

				if (this.id!=albumID) items.push({
					title: html,
					fn: () => ProrigoGallery.goto(this.id)
				})
...
contextMenu.photoTitle = function(albumID, photoID, e) {
	...
	if (data.content!==false && data.num>1) {
		...
		$.each(data.content, function(index) {
			...
			let html = ProrigoGallery.html`<img class='cover' width='16' height='16' src='$${ this.thumbUrl }'><div class='title'>$${ this.title }</div>`

			if (this.id!=photoID) items.push({
				title: html,
				fn: () => ProrigoGallery.goto(albumID + '/' + this.id)
			})
...
contextMenu.photoMore = function(photoID, e) {
	...
	let showDownload = ProrigoGallery.publicMode===false || ((album.json && album.json.downloadable && album.json.downloadable==='1') && ProrigoGallery.publicMode===true)
...
contextMenu.sharePhoto = function(photoID, e) {
	...
	let items = [
		...
		{ title: build.iconic('dropbox', iconClass) + 'Dropbox', visible: ProrigoGallery.publicMode===false, fn: () => photo.share(photoID, 'dropbox') },
		{ title: build.iconic('link-intact') + 'Direct Link', fn: () => window.open(photo.getDirectLink()) },
		{ },
		{ title: build.iconic('ban') + 'Make Private', visible: ProrigoGallery.publicMode===false, fn: () => photo.setPublic(photoID) }
	]

	if (ProrigoGallery.publicMode===true) items.splice(7, 1)
...
contextMenu.shareAlbum = function(albumID, e) {
	...
	let items = [
		...
		{ },
		{ title: build.iconic('pencil') + 'Edit Sharing', visible: ProrigoGallery.publicMode===false, fn: () => album.setPublic(albumID, true, e) },
		{ title: build.iconic('ban') + 'Make Private', visible: ProrigoGallery.publicMode===false, fn: () => album.setPublic(albumID, false) }
	]

	if (ProrigoGallery.publicMode===true) items.splice(5, 1)
