 ...
 * @description This file takes a look at your ProrigoGallery-configuration and displays all errors it can find.
 */
...
require($ProrigoGallery . 'php/helpers/hasPermissions.php');
...
// Permissions
if (hasPermissions(ProrigoGallery_UPLOADS_BIG)===false)    $error .= ('Error: \'uploads/big\' is missing or has insufficient read/write privileges' . PHP_EOL);
if (hasPermissions(ProrigoGallery_UPLOADS_MEDIUM)===false) $error .= ('Error: \'uploads/medium\' is missing or has insufficient read/write privileges' . PHP_EOL);
if (hasPermissions(ProrigoGallery_UPLOADS_THUMB)===false)  $error .= ('Error: \'uploads/thumb\' is missing or has insufficient read/write privileges' . PHP_EOL);
if (hasPermissions(ProrigoGallery_UPLOADS_IMPORT)===false) $error .= ('Error: \'uploads/import\' is missing or has insufficient read/write privileges' . PHP_EOL);
if (hasPermissions(ProrigoGallery_UPLOADS)===false)        $error .= ('Error: \'uploads/\' is missing or has insufficient read/write privileges' . PHP_EOL);
if (hasPermissions(ProrigoGallery_DATA)===false)           $error .= ('Error: \'data/\' is missing or has insufficient read/write privileges' . PHP_EOL);
...
// Load config
if (!file_exists(ProrigoGallery_CONFIG_FILE)) exit('Error: Configuration not found. Please install ProrigoGallery for additional tests');
...
// Check mysql version
if ($database->server_version<50500) echo('Warning: ProrigoGallery uses the GBK charset to avoid sql injections on your MySQL version. Please update to MySQL 5.5 or higher to enable UTF-8 support.' . PHP_EOL);
...
// Output
if ($error==='') echo('No critical problems found. ProrigoGallery should work without problems!' . PHP_EOL);
else             echo $error;
...
if ((isset($_SESSION['login'])&&$_SESSION['login']===true)&&
	...
	// Load json
	$json = file_get_contents(ProrigoGallery_SRC . 'package.json');
	...
	echo('ProrigoGallery Version:  ' . $json['version'] . PHP_EOL);
