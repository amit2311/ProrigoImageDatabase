...
**Warning**: It's no longer possible to update from ProrigoGallery versions older than 2.7.

**Warning**: Plugins which use the plugin API of ProrigoGallery must be updated to work with the new back-end.
...
- `Improved` ProrigoGallery update site now with SSL (#317)
...
- `New` Identifier to prevent login of multiple ProrigoGallery-instances (#344)
...
**Warning**: You need to enter a new username and password when upgrading from a previous version. Your installation is accessible for everyone till you enter a new login by visiting your ProrigoGallery. Both fields are now stored in a secure way. Legacy md5 code has been removed.
...
**Deprecated**: Photos uploaded with ProrigoGallery v1.1 or older aren't supported anymore. Thumbnails  fail to load on high-res screens.
...
- `New` Database table prefix for multiple ProrigoGallery installations (#196)
...
- `New` Compile ProrigoGallery using Grunt (with npm and bower)
