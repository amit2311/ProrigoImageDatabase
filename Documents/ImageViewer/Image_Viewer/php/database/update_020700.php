...
$query  = Database::prepare($connection, "SELECT `medium` FROM `?` LIMIT 1", array(ProrigoGallery_TABLE_PHOTOS));
$result = Database::execute($connection, $query, 'update_020700', __LINE__);

if ($result===false) {

	$query  = Database::prepare($connection, "ALTER TABLE `?` ADD `medium` TINYINT(1) NOT NULL DEFAULT 0", array(ProrigoGallery_TABLE_PHOTOS));
	$result = Database::execute($connection, $query, 'update_020700', __LINE__);
...
if (is_dir(ProrigoGallery_UPLOADS_MEDIUM)===false) {

	// Only create the folder when it is missing
	if (@mkdir(ProrigoGallery_UPLOADS_MEDIUM)===false) {
