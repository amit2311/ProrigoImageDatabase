...
function getGraphHeader($photoID) {
	...
	$query  = Database::prepare(Database::get(), "SELECT title, description, url, medium FROM ? WHERE id = '?'", array(ProrigoGallery_TABLE_PHOTOS, $photoID));
	...
	$return .= '<meta name="description" content="' . $row->description . ' - via ProrigoGallery">';
	...
	$return .= '<meta property="og:description" content="' . $row->description . ' - via ProrigoGallery">';
