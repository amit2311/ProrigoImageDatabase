...
final class Import {
	...
	/**
	 * Creates an array similar to a file upload array and adds the photo to ProrigoGallery.
	...
	public function url($urls, $albumID = 0) {
		...
		foreach ($urls as &$url) {
			...
			$tmp_name = ProrigoGallery_DATA . $filename;
	...
	public function server($path, $albumID = 0) {
		...
		// Parse path
		if (!isset($path))           $path = ProrigoGallery_UPLOADS_IMPORT;
		...
		// Skip folders of ProrigoGallery
		if ($path===ProrigoGallery_UPLOADS_BIG||($path . '/')===ProrigoGallery_UPLOADS_BIG||
			$path===ProrigoGallery_UPLOADS_MEDIUM||($path . '/')===ProrigoGallery_UPLOADS_MEDIUM||
			$path===ProrigoGallery_UPLOADS_THUMB||($path . '/')===ProrigoGallery_UPLOADS_THUMB) {
				Log::error(Database::get(), __METHOD__, __LINE__, 'The given path is a reserved path of ProrigoGallery (' . $path . ')');
				return false;
		}
		...
		foreach ($files as $file) {
			...
			if (@exif_imagetype($file)!==false) {
				...
				if ($newAlbumID===false) {
					$error = true;
					Log::error(Database::get(), __METHOD__, __LINE__, 'Could not create album in ProrigoGallery (' . $newAlbumID . ')');
					continue;
				}
