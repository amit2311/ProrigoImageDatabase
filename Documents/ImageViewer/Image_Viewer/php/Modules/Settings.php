...
final class Settings {
	...
	/**
	 * @return array Returns the settings of ProrigoGallery.
	 */
	public static function get() {
		...
		// Execute query
		$query    = Database::prepare(Database::get(), "SELECT * FROM ?", array(ProrigoGallery_TABLE_SETTINGS));
	...
	private static function set($key, $value, $row = false) {
		...
		if ($row===false) {
			...
			$query = Database::prepare(Database::get(), "UPDATE ? SET value = '?' WHERE `key` = '?'", array(ProrigoGallery_TABLE_SETTINGS, $value, $key));

		} elseif ($row===true) {

			// Do not prepare $value because it has already been escaped or is a true statement
			$query = Database::prepare(Database::get(), "UPDATE ? SET value = '$value' WHERE `key` = '?'", array(ProrigoGallery_TABLE_SETTINGS, $key));
