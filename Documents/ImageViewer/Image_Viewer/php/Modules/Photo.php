...
namespace ProrigoGallery\Modules;
...
final class Photo {
	...
	public function add(array $files, $albumID = 0, $returnOnError = false) {
		...
		// Check permissions
		if (hasPermissions(ProrigoGallery_UPLOADS)===false||
			hasPermissions(ProrigoGallery_UPLOADS_BIG)===false||
			hasPermissions(ProrigoGallery_UPLOADS_THUMB)===false) {
		...
		$path       = ProrigoGallery_UPLOADS_BIG . $photo_name;
		...
		$values = array(ProrigoGallery_TABLE_PHOTOS, $id, $info['title'], $photo_name, $info['description'], $info['tags'], $info['type'], $info['width'], $info['height'], $info['size'], $info['iso'], $info['aperture'], $info['make'], $info['model'], $info['shutter'], $info['focal'], $info['takestamp'], $path_thumb, $albumID, $public, $star, $checksum, $medium);
	...
	private function exists($checksum, $photoID = null) {
		...
		if (isset($photoID)) $query = Database::prepare(Database::get(), "SELECT id, url, thumbUrl, medium FROM ? WHERE checksum = '?' AND id <> '?' LIMIT 1", array(ProrigoGallery_TABLE_PHOTOS, $checksum, $photoID));
		else                 $query = Database::prepare(Database::get(), "SELECT id, url, thumbUrl, medium FROM ? WHERE checksum = '?' LIMIT 1", array(ProrigoGallery_TABLE_PHOTOS, $checksum));
		...
		if ($result->num_rows===1) {
			...
			$return = array(
				...
				'path'       => ProrigoGallery_UPLOADS_BIG . $result->url,
	...
	private function createThumb($url, $filename, $type, $width, $height) {
		...
		$newUrl    = ProrigoGallery_UPLOADS_THUMB . $photoName[0] . '.jpeg';
		$newUrl2x  = ProrigoGallery_UPLOADS_THUMB . $photoName[0] . '@2x.jpeg';
	...
	private function createMedium($url, $filename, $width, $height) {
		...
		// Check permissions
		if (hasPermissions(ProrigoGallery_UPLOADS_MEDIUM)===false) {
		...
		if (($error===false)&&
			...
			$newUrl = ProrigoGallery_UPLOADS_MEDIUM . $filename;
	...
	public static function prepareData(array $data) {
		...
		// Parse medium
		if ($data['medium']==='1') $photo['medium'] = ProrigoGallery_URL_UPLOADS_MEDIUM . $data['url'];
		else                       $photo['medium'] = '';

		// Parse paths
		$photo['thumbUrl'] = ProrigoGallery_URL_UPLOADS_THUMB . $data['thumbUrl'];
		$photo['url']      = ProrigoGallery_URL_UPLOADS_BIG . $data['url'];
	...
	public function get($albumID) {
		...
		// Get photo
		$query  = Database::prepare(Database::get(), "SELECT * FROM ? WHERE id = '?' LIMIT 1", array(ProrigoGallery_TABLE_PHOTOS, $this->photoIDs));
		...
		// Parse medium
		if ($photo['medium']==='1') $photo['medium'] = ProrigoGallery_URL_UPLOADS_MEDIUM . $photo['url'];
		else                        $photo['medium'] = '';

		// Parse paths
		$photo['url']      = ProrigoGallery_URL_UPLOADS_BIG . $photo['url'];
		$photo['thumbUrl'] = ProrigoGallery_URL_UPLOADS_THUMB . $photo['thumbUrl'];
		...
		if ($albumID!='false') {
			...
			if ($photo['album']!=='0') {
				...
				// Get album
				$query  = Database::prepare(Database::get(), "SELECT public FROM ? WHERE id = '?' LIMIT 1", array(ProrigoGallery_TABLE_ALBUMS, $photo['album']));
	...
	public function getArchive() {
		...
		// Get photo
		$query  = Database::prepare(Database::get(), "SELECT title, url FROM ? WHERE id = '?' LIMIT 1", array(ProrigoGallery_TABLE_PHOTOS, $this->photoIDs));
		$photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
		...
		header("Content-Length: " . filesize(ProrigoGallery_UPLOADS_BIG . $photo->url));

		// Send file
		readfile(ProrigoGallery_UPLOADS_BIG . $photo->url);
	...
	public function setTitle($title = 'Untitled') {
		...
		// Set title
		$query  = Database::prepare(Database::get(), "UPDATE ? SET title = '?' WHERE id IN (?)", array(ProrigoGallery_TABLE_PHOTOS, $title, $this->photoIDs));
		$result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
	...
	public function setDescription($description) {
		...
		// Set description
		$query  = Database::prepare(Database::get(), "UPDATE ? SET description = '?' WHERE id IN ('?')", array(ProrigoGallery_TABLE_PHOTOS, $description, $this->photoIDs));
		$result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
	...
	public function setStar() {
		...
		// Get photos
		$query  = Database::prepare(Database::get(), "SELECT id, star FROM ? WHERE id IN (?)", array(ProrigoGallery_TABLE_PHOTOS, $this->photoIDs));
		$photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
		...
		while ($photo = $photos->fetch_object()) {
			...
			// Set star
			$query  = Database::prepare(Database::get(), "UPDATE ? SET star = '?' WHERE id = '?'", array(ProrigoGallery_TABLE_PHOTOS, $star, $photo->id));
			$result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
	...
	public function getPublic($password) {
		...
		// Get photo
		$query  = Database::prepare(Database::get(), "SELECT public, album FROM ? WHERE id = '?' LIMIT 1", array(ProrigoGallery_TABLE_PHOTOS, $this->photoIDs));
		$photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
	...
	public function setPublic() {
		...
		// Get public
		$query  = Database::prepare(Database::get(), "SELECT public FROM ? WHERE id = '?' LIMIT 1", array(ProrigoGallery_TABLE_PHOTOS, $this->photoIDs));
		$photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
		...
		// Set public
		$query  = Database::prepare(Database::get(), "UPDATE ? SET public = '?' WHERE id = '?'", array(ProrigoGallery_TABLE_PHOTOS, $public, $this->photoIDs));
		$result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
	...
	function setAlbum($albumID) {
		...
		// Set album
		$query  = Database::prepare(Database::get(), "UPDATE ? SET album = '?' WHERE id IN (?)", array(ProrigoGallery_TABLE_PHOTOS, $albumID, $this->photoIDs));
		$result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
	...
	public function setTags($tags) {
		...
		// Set tags
		$query  = Database::prepare(Database::get(), "UPDATE ? SET tags = '?' WHERE id IN (?)", array(ProrigoGallery_TABLE_PHOTOS, $tags, $this->photoIDs));
		$result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
	...
	public function duplicate() {
		...
		// Get photos
		$query  = Database::prepare(Database::get(), "SELECT id, checksum FROM ? WHERE id IN (?)", array(ProrigoGallery_TABLE_PHOTOS, $this->photoIDs));
		$photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
		...
		while ($photo = $photos->fetch_object()) {
			...
			// Duplicate entry
			$values = array(ProrigoGallery_TABLE_PHOTOS, $id, ProrigoGallery_TABLE_PHOTOS, $photo->id);
	...
	public function delete() {
		...
		// Get photos
		$query  = Database::prepare(Database::get(), "SELECT id, url, thumbUrl, checksum FROM ? WHERE id IN (?)", array(ProrigoGallery_TABLE_PHOTOS, $this->photoIDs));
		$photos = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
		...
		while ($photo = $photos->fetch_object()) {
			...
			if ($this->exists($photo->checksum, $photo->id)===false) {
				...
				// Delete big
				if (file_exists(ProrigoGallery_UPLOADS_BIG . $photo->url)&&!unlink(ProrigoGallery_UPLOADS_BIG . $photo->url)) {
					Log::error(Database::get(), __METHOD__, __LINE__, 'Could not delete photo in uploads/big/');
					$error = true;
				}

				// Delete medium
				if (file_exists(ProrigoGallery_UPLOADS_MEDIUM . $photo->url)&&!unlink(ProrigoGallery_UPLOADS_MEDIUM . $photo->url)) {
					Log::error(Database::get(), __METHOD__, __LINE__, 'Could not delete photo in uploads/medium/');
					$error = true;
				}

				// Delete thumb
				if (file_exists(ProrigoGallery_UPLOADS_THUMB . $photo->thumbUrl)&&!unlink(ProrigoGallery_UPLOADS_THUMB . $photo->thumbUrl)) {
					Log::error(Database::get(), __METHOD__, __LINE__, 'Could not delete photo in uploads/thumb/');
					$error = true;
				}

				// Delete thumb@2x
				if (file_exists(ProrigoGallery_UPLOADS_THUMB . $thumbUrl2x)&&!unlink(ProrigoGallery_UPLOADS_THUMB . $thumbUrl2x)) {
			...
			// Delete db entry
			$query  = Database::prepare(Database::get(), "DELETE FROM ? WHERE id = '?'", array(ProrigoGallery_TABLE_PHOTOS, $photo->id));
			$result = Database::execute(Database::get(), $query, __METHOD__, __LINE__);
