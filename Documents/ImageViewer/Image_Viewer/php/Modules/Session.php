...
final class Session {
	...
	/**
	 * Reads and returns information about the ProrigoGallery installation.
	...
	public function init($public = true) {
		...
		// Path to ProrigoGallery for the server-import dialog
		$return['config']['location'] = ProrigoGallery;
		...
		if ($public===false) {
			...
			// Logged in
			$return['status'] = ProrigoGallery_STATUS_LOGGEDIN;

		} else {

			// Logged out
			$return['status'] = ProrigoGallery_STATUS_LOGGEDOUT;
